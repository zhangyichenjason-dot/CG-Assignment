...
public:
	...
	// 创建支持透明混合和关闭深度写入的PSO-用于远景类效果
    void createTransparentPSO(Core* core, std::string name, ID3DBlob* vs, ID3DBlob* ps, D3D12_INPUT_LAYOUT_DESC layout)
    {
        if (psos.find(name) != psos.end())
        {
            return;
        ...
        blendDesc.AlphaToCoverageEnable = FALSE;
        blendDesc.IndependentBlendEnable = FALSE;
        const D3D12_RENDER_TARGET_BLEND_DESC transparentBlend = {
            TRUE, FALSE,
        ...
        for (int i = 0; i < D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT; i++)
        {
            blendDesc.RenderTarget[i] = transparentBlend;
        }
        desc.BlendState = blendDesc;
